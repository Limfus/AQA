...
for car, attributes in car_data.items():
    year, engine_volume, price = attributes[1], attributes[2], attributes[4]
...
# Sort by price (index 4 in the tuple)
filtered_cars.sort(key=lambda x: x[1][4])

# Get the first five elements
result = filtered_cars[:5]

# Print the first five cars that satisfy the search criteria, sorted by price
for car, attributes in result:
    print(f"Car: {car}, Color: {attributes[0]}, Year: {attributes[1]}, Engine Volume: {attributes[2]}, "
          f"Car Type: {attributes[3]}, Price: {attributes[4]}")
